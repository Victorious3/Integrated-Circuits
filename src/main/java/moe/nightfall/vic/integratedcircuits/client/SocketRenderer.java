...
public class SocketRenderer extends PartRenderer<ISocket> {
	...
	public void renderStatic(Transformation t, int orient) {
		...
		if (socket != null && socket.getGate() != null)
			IntegratedCircuitsAPI.getGateRegistry().getRenderer(socket.getGate().getClass()).renderStatic(t, orient);
	}

	public void renderDynamic(Transformation t) {
		if (socket != null && socket.getGate() != null) {
			Transformation rotation = Rotation.sideOrientation(socket.getSide(), socket.getRotation()).at(
					Vector3.center);
			IntegratedCircuitsAPI.getGateRegistry().getRenderer(socket.getGate().getClass())
				.renderDynamic(rotation.with(t));
		} else if (socket != null) {
			...
			if ((stack = player.getCurrentEquippedItem()) != null && stack.getItem() instanceof IGateItem) {
				...
				if (mop.typeOfHit == MovingObjectType.BLOCK && mop.blockX == pos.x && mop.blockY == pos.y
					...
					String gateID = ((IGateItem) stack.getItem()).getGateID(stack, Minecraft.getMinecraft().thePlayer,
							pos);
					...
					CCRenderState.alphaOverride = (int) (120 + 20 * Math
						.sin((ClientProxy.clientTicks + partialTicks) / 5D));
